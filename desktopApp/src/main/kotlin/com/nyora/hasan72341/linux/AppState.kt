...
import com.nyora.linux.bridge.CatalogEntry
import com.nyora.linux.bridge.CatalogResponse
import com.nyora.linux.bridge.CategoriesResponse
import com.nyora.linux.bridge.CategoryDto
import com.nyora.linux.bridge.DownloadDto
...
import com.nyora.linux.bridge.NetworkSettingsDto
import com.nyora.linux.bridge.NetworkSettingsResponse
import com.nyora.linux.bridge.StatsResponse
import com.nyora.linux.bridge.SupabaseLocalDataResponse
import com.nyora.linux.bridge.OtaStatusResponse
import com.nyora.linux.bridge.SupabaseOkResponse
import com.nyora.linux.bridge.SupabaseStatusResponse
...
class AppState(
    ...
    // MUST be declared before init{} (which calls loadPrefs()): property initializers run in
    // source order, so a Json defined lower in the class would be null when loadPrefs() reads it.
    ...
    var showUnreadBadge   by mutableStateOf(true)
    var quickFilters      by mutableStateOf(true)
    var progressIndicator by mutableStateOf("circular") // "circular"|"bar"|"off"
    var pagesThumbnailTab by mutableStateOf(true)
    var sidebarLabels     by mutableStateOf(true)
    ...
    var readerScaleMode       by mutableStateOf("fit_center") // fit_center|fit_width|fit_height|fill
    var readerRtl             by mutableStateOf(false)
    var webtoonZoom           by mutableStateOf(true)
    var webtoonZoomOut        by mutableStateOf(0f)   // 0..0.5
    var webtoonGaps           by mutableStateOf(false)
    var tapZones              by mutableStateOf(true)
    var tapZonesLtr           by mutableStateOf(false)
    var invertNavigation      by mutableStateOf(false)
    ...
    var translationResponseScale by mutableStateOf(1.0f) // 0.75..1.6
    var translationTier       by mutableStateOf("tuned") // fast|tuned|balanced|quality
    var debugHud              by mutableStateOf(false)
    ...
    var newChapterNotifications by mutableStateOf(true)

    // ── Settings: Downloads ──────────────────────────────────────────────────────────
    var maxConcurrentDownloads by mutableStateOf(3)
    var downloadFormat         by mutableStateOf("auto") // auto|folder|cbz|zip
    ...
    /** Dismiss the first-run welcome and remember it so it never shows again. */
    fun finishOnboarding() {
        runCatching { configDir().mkdirs(); onboardedMarker().writeText("1") }
        showWelcome = false
    }
    ...
    fun browseManga(source: MangaSource, page: Int = 1) {
        ...
        exploreError = null
        exploreLoading = true
        if (page == 1) exploreManga = emptyList()
        scope.launch {
            runCatching {
    ...
    /**
     * Switches the OCR *source* language(s) (Tesseract codes, e.g. "jpn", "kor",
     * "chi_sim+chi_sim_vert", "ara"). The page's text language must match or OCR
     * produces garbage. Clears cached translations and re-OCRs the current page.
     */
    ...
    // ── Library refresh ───────────────────────────────────────────────────────
        ...
        fun refreshLibrary() {
        scope.launch { refreshLibrarySuspend() }
    }

    suspend fun refreshLibrarySuspend() = withContext(Dispatchers.IO) {
        val favs = facade.favourites()
        val hist = facade.history(100)
        val bms  = facade.bookmarks()
        val upd  = facade.updates()
        withContext(Dispatchers.Main) {
            favourites = favs
            history = hist
            bookmarks = bms
    ...
    private var backfillingCovers = false
    /** History/favourites synced from another device often arrive with a BLANK
     *  cover (the recording device never captured it). Search each item's source
     *  by title and persist the best match's cover, so thumbnails fill in — the
     *  same backfill mac/iOS do. No-op once everything has a cover. */
    fun backfillMissingCovers() {
        if (backfillingCovers) return
        val needing = (history.map { it.manga } + favourites)
            .distinctBy { it.id }
            .filter { it.coverUrl.isBlank() && it.title.isNotBlank() }
        if (needing.isEmpty()) return
        scope.launch {
            backfillingCovers = true
            var filled = 0
            try {
                for (manga in needing) {
                    val src = sourceFor(manga) ?: continue
                    runCatching {
                        val svc = withContext(Dispatchers.IO) { facade.openExtension(src) }
                        val result = withContext(Dispatchers.IO) { svc.search(manga.title, 1) }
                        val cover = result.entries.firstOrNull {
                                it.title.equals(manga.title, ignoreCase = true) && it.coverUrl.isNotBlank()
                            }?.coverUrl
                            ?: result.entries.firstOrNull { it.coverUrl.isNotBlank() }?.coverUrl
                        if (!cover.isNullOrBlank()) {
                            withContext(Dispatchers.IO) { facade.upsertManga(manga.copy(coverUrl = cover)) }
                            filled++
                        }
                    }
                }
    ...
    fun sourceFor(manga: Manga): MangaSource? {
        ...
        // Synced manga carry a canonical "JS_<ID>" ref while installed sources
        // use "parser:<ID>" — so match on the BARE id too (strip JS_/parser:),
        // mirroring the shared toOpenableSourceId(). Without this, every history
        // /favourite item synced from another device fails to resolve its source.
    ...
    fun coverProxyUrl(coverUrl: String, source: MangaSource? = null): String {
        ...
        if (coverUrl.startsWith(imageBaseUrl)) return coverUrl  // already proxied
    ...
    private fun configDir(): File {
        ...
        val dir = if (os.contains("mac") || os.contains("darwin")) {
    ...
    private fun loadPrefs() {
        runCatching {
            ...
            // Appearance / lists
            showUnreadBadge      = dto.showUnreadBadge
            quickFilters         = dto.quickFilters
            progressIndicator    = dto.progressIndicator
            pagesThumbnailTab    = dto.pagesThumbnailTab
            sidebarLabels        = dto.sidebarLabels
            // Reader extras
            ...
            webtoonZoomOut       = dto.webtoonZoomOut
            webtoonGaps          = dto.webtoonGaps
            tapZones             = dto.tapZones
            tapZonesLtr          = dto.tapZonesLtr
            invertNavigation     = dto.invertNavigation
            // Translation extras
            translateEnabled     = dto.translateEnabled
            translateTarget      = dto.translateTarget
            translateLangs       = dto.translateLangs
            translationResponseScale = dto.translationResponseScale
            translationTier      = dto.translationTier
            debugHud             = dto.debugHud
            // Notifications
            newChapterNotifications = dto.newChapterNotifications
            // Downloads
        ...
        scope.launch(Dispatchers.IO) {
            runCatching {
                ...
                val dto = AppPrefs(
                    ...
                    pagesThumbnailTab = pagesThumbnailTab,
                    ...
                    tapZones = tapZones,
                    tapZonesLtr = tapZonesLtr,
                    invertNavigation = invertNavigation,
                    // Translation extras
                    translateEnabled = translateEnabled,
                    translateTarget = translateTarget,
                    translateLangs = translateLangs,
                    translationResponseScale = translationResponseScale,
    ...
    fun cloudSignInWithGoogle() {
        ...
        scope.launch {
            ...
            runCatching {
                ...
                showStatus("Opening your browser… if it doesn't appear, the sign-in link is copied to your clipboard.")
    ...
    /** Re-open the Google sign-in URL (used by the "didn't open?" fallback). */
    ...
    fun openExternalUrl(url: String) {
        ...
        // `rundll32` have no such constraint.
    ...
    private data class AppPrefs(
        ...
        val quickFilters: Boolean = true,
        val progressIndicator: String = "circular",
        val pagesThumbnailTab: Boolean = true,
        val sidebarLabels: Boolean = true,
        // Reader extras (also persist the previously-transient ones)
        ...
        val webtoonZoomOut: Float = 0f,
        val webtoonGaps: Boolean = false,
        val tapZones: Boolean = true,
        val tapZonesLtr: Boolean = false,
        val invertNavigation: Boolean = false,
        // Translation extras
        val translateEnabled: Boolean = false,
        val translateTarget: String = "en",
        val translateLangs: String = "jpn+eng",
        val translationResponseScale: Float = 1.0f,
        val translationTier: String = "tuned",
